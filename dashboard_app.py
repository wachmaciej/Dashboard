...
def create_yoy_trends_chart(data, selected_years, selected_quarters, selected_channels=None, selected_listings=None, selected_products=None):
    """
    Create a Plotly line chart for YOY Weekly Revenue Trends with custom tooltip and markers.
    """
    filtered = data.copy()
    if selected_years:
...
def create_sales_channel_chart(data, selected_years, selected_quarter):
    """
    Create a Plotly bar chart for Sales Revenue by Sales Channel.
    """
    filtered = data.copy()
    if selected_years:
        filtered = filtered[filtered["Year"].isin(selected_years)]
...
def create_listings_chart(data, selected_years, selected_quarters, selected_listings):
    """
    Create a Plotly line chart for Weekly Sales by Listing with custom tooltip and markers.
    """
    filtered = data.copy()
    if selected_years:
        filtered = filtered[filtered["Year"].isin(selected_years)]
    # Filter by quarters if user did not choose "All Quarters"
...
def create_pivot_table(data, selected_years, sort_order):
    """
    Create a pivot table of Revenue by Listing and Week.
    """
    filtered = data.copy()
    if selected_years:
        filtered = filtered[filtered["Year"].isin(selected_years)]
...
def create_sku_line_chart(data, sku_text, selected_years, selected_quarter):
    """
    Create a Plotly line chart for a specific SKU based on the 'Product SKU' column.
    """
...
def create_daily_price_chart(data, listing, selected_years, selected_quarters, selected_channels):
    ...
    This function reindexes the data to include all days in the range (from the first to the last day with sales)
    and then interpolates missing values to ensure a continuous line even for days without sales.
    """
    ...
    for yr in selected_years:
        ...
        # Reindex the dataframe so that every day in the range is present
        df_year = df_year.set_index("Day").reindex(range(start_day, end_day + 1))
        df_year.index.name = "Day"
        # Interpolate missing Average Price values for days with no sales
...
with tabs[0]:
    ...
    with st.expander("KPI Filters", expanded=True):
        # Compute the last full week (Monday to Sunday) based on today's date
...
with tabs[1]:
    ...
    if unique_weeks_current.empty:
        ...
        # Keep the numeric columns for sorting
        ...
        # Compute percentage change for the 4-week and 1-week comparisons
        ...
        if "sort_order" not in st.session_state:
            st.session_state.sort_order = "desc"  # default descending
        ...
        # Sort based on the numeric revenue value (ignoring the percentage calculation)
        ...
        # --- Helper function to combine revenue and percentage change ---
        def combine_revenue_pct(row, revenue_col, pct_col):
            """
            Combines a revenue value and its percentage change into a single formatted string.
            """
        ...
        # Create new combined columns for current year's metrics
...
with tabs[2]:
    ...
    with st.expander("Daily Price Filters", expanded=True):
        # Set default years to 2024 and 2025 if they are available; otherwise default to current_year
    ...
    # Top charts for our main listings
    ...
    # --- NEW: Additional Daily Prices Comparison Chart ---
    st.markdown("## Daily Prices Comparison")
    with st.expander("Comparison Chart Filters", expanded=True):
        # These filters work independently from the above filters.
    ...
    # For the comparison chart we want to show data for all quarters
...
with tabs[6]:
    ...
    with st.expander("Chart Filters", expanded=True):
        ...
        # Change the quarter selection from a selectbox to a multiselect:
